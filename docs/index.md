# Documentação Técnica — Desbravador Software

Bem-vindo ao portal de documentação técnica da Desbravador Software Ltda.

Aqui você encontra os requisitos de hardware, software e configuração para todos os sistemas Desbravador, tanto para instalação local (on-premise) quanto em nuvem (cloud).

---

## Sistemas disponíveis

<HomeTable />

---

## Periféricos Homologados

Páginas compartilhadas entre todos os sistemas:

- 🔒 [Fechaduras e tarifadores homologados](/perifericos/fechaduras-homologadas)
- 🖨️ [Impressoras fiscais e não fiscais homologadas](/perifericos/impressoras-homologadas)
- 💳 [Pinpads homologados](/perifericos/pinpads-homologados)
- 💳 [Sistemas de TEF homologados](/perifericos/tef-homologados)
- 📱 [Dispositivos iPDV e PDV homologados](/perifericos/dispositivos-ipdv-pdv)

---

## Suporte

**Desbravador Software Ltda.**  
🌐 [www.desbravador.com.br](https://www.desbravador.com.br)
